...
The LaunchAgent (named `com.<yourusername>.granola-sync`, generated by `install.sh`) runs the sync every **Tuesday at 6:07 AM**. Logs go to:
...
```bash
launchctl start com.$(whoami).granola-sync
```
...
```bash
launchctl unload ~/Library/LaunchAgents/com.$(whoami).granola-sync.plist
```

To re-enable:
```bash
launchctl load -w ~/Library/LaunchAgents/com.$(whoami).granola-sync.plist
```
...
├── install.sh         # one-time setup — generates plist and installs LaunchAgent
├── requirements.txt   # python-docx, cryptography
├── README.md
└── .gitignore
```
...
Output documents (default: `~/Documents/Granola Notes/`):
```
~/Documents/Granola Notes/
├── Project Alpha/
│   ├── 2026-01-06_Kickoff meeting.docx
│   └── ...
├── Sales/
│   ├── 2026-04-02_Intro call - Acme.docx
│   └── ...
├── _Unfiled/
│   └── ...
└── ...
